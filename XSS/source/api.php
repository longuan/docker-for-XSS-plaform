...
if($id){
	...
	if(!$judgeCookie || $cookieExisted<=0){
        ...
        $Getcookie=$content['cookie'];
				...
				SendSMS('13800138000','123456',$userInfo['phone'],"尊敬的".$userInfo['userName']."，您在".URL_ROOT." 预订的猫饼干，Cookie:{$Getcookie}已经到货！详情请登录：".URL_ROOT." 查看！");
				//参数:发送的飞信号 飞信密码
				}
                if($userInfo['email'] && $msg[0]==1){
                $domain=explode("//", URL_ROOT);
                SendMail($userInfo['email'],"notification @ ".$domain[1] ,"尊敬的 ".$userInfo['userName']." ，您在 ".$domain[1]." 预订的零食<br>Cookie:{$Getcookie}<br>已经到货！<br>详情请登录：".URL_ROOT." 查看。");//Getcookie在上面的变量里
                }
